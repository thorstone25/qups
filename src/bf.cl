...
// # include "interpd.cl" // samplers using constant sizing
...
kernel void DAS(volatile global U2 * y, 
    const global U4 * pi, const global U4 * pr,
    const global U4 * pv, const global U4 * nv, 
    const global U2 * a,  const global U  * cinv, const global ulong * acstride, 
    const global U2 * x, const int flag, const global U t0fsfc[2]) {
    ...
    for(size_t i = tid; i < I; i += kI){        
        ...
        for(size_t m = 0; m < M; ++m){
            for(size_t n = 0; n < N; ++n){
                ...
                rv = pi[i].xyz - pv[m].xyz; // (virtual) transmit to pixel vector

                dv = QUPS_VS ? // tx path length
                    copysign(length(rv), (QUPS_DV ? 1.f : dot(rv, nv[m].xyz))) // virtual source
                    : dot(rv, nv[m].xyz); // plane wave
