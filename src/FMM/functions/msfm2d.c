...
void mexFunction( int nlhs, mxArray *plhs[],
    ...
    if (listval != NULL) listval[0]=(double*)malloc(2 * sizeof(double));

    success = (Frozen != NULL) && (neg_listx != NULL) && (neg_listy != NULL) && (listprop != NULL) && ((listval != NULL) && (listval[0] != NULL));
    if(Ed) success = success && (neg_listo != NULL);
    if(!success){
        destroy_list(listval, listprop);
        if(neg_listx != NULL) free(neg_listx);
        if(neg_listy != NULL) free(neg_listy);
        if(Ed) if(neg_listo != NULL) free(neg_listo);
        if(Frozen != NULL) free(Frozen);
    ...
    for (z=0; z<dims_sp[1]; z++) {
        ...
        for (k=0; k<4; k++) {
             ...
             *picture  */
            int idims[2] = {dims[0], dims[1]};
            if(isntfrozen2d(i, j, idims, Frozen)) {
    ...
    if(!success) npixels = 0; // we had an error: skip the for loop
    ...
    for (itt=0; itt<npixels; itt++) {
        ...
        list_remove_replace(listval, listprop, index); // this ~should~ be safe since it's a shrinking reallocation
        neg_listv=listval[listprop[1]-1];
        if(index<(neg_pos-1)) {
        ...
        for (k=0;k<4;k++) {
            ...
            if(isntfrozen2d(i, j, idims, Frozen)) {
                ...
                Tt=CalculateDistance(T, F[IJ_index], idims, i, j, usesecond, usecross, Frozen);
				        
				if(Ed) {
                    Ty=CalculateDistance(Y, 1, idims, i, j, usesecond, usecross, Frozen);
                }
    ...
    if(!success) mexErrMsgTxt("Out of memory.");
}
