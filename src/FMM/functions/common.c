...
int myiszero(double a){ return a*a<eps; }
...
__inline bool isntfrozen3d(int i, int j, int k, int *dims, bool *Frozen) {
    ...
    return (i>=0)&&(j>=0)&&(k>=0)&&(i<dims[0])&&(j<dims[1])&&(k<dims[2])&&(!Frozen[mindex3(i, j, k, dims[0], dims[1])]);
}
...
void destroy_list(double ** listval, int *listprop) {
    ...
    /* Free memory */
    for(i=0;i<list_orde;i++) { if(listval[i] != NULL) free(listval[i]); }
    if(listval != NULL) free(listval);
    if(listprop != NULL) free(listprop);
}

bool list_add(double ** listval, int *listprop, double val) {
    /* List parameters */
...
void list_remove_replace(double ** listval, int *listprop, int index) {
    ...
    if(index<(list_length-1)) {
        ...
        for (i=(list_orde-1); i>0; i--) {
            listp=(int)floor(((double)listp)/2);
            if(val<listval[i-1][listp]) {
                listval[i-1][listp]=val; 
            } else {
                break; 
            }
        }
...
{
    return (i>=0)&&(j>=0)&&(i<dims[0])&&(j<dims[1])&&(!Frozen[i+j*dims[0]]);
}
